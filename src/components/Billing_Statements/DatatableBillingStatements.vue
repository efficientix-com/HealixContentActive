<template>
  ...
  import { data_test_billing_statements } from "./data_test_billing_statements";
  ...
  export default {
    ...
    mounted() {
      this.datatable = new DataTable("#datatable_Billing_Invoice", {
        responsive: true,
        data: data_test_billing_statements(),
        columnDefs: [
          { targets: [2, 3], width: "10rem" },
  
          { targets: [0], width: "0.8rem" },
        ],
        order: [],
        columns: [
          {
          ...
          {
            data: "statement_recipient",
            title: "Statement Recipient",
          }
